...
class NodeService(INodeService):
    ...
    def _create_track(self, project_id: str, name: str,
                      track_type: str) -> ToolResponse:
        project = self._manager.get_project(project_id)
        if not project:
            return ToolResponse("error", None,
                                f"Project '{project_id}' not found.")

        # NodeFactory 从 Manager 获取，因为它与后端相关
        node_factory = self._manager.node_factory
        command = CreateTrackCommand(project.router, node_factory, track_type,
                                     name)
